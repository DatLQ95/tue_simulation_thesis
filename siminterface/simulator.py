...
from coordsim.trace_processor.percentage_trace_processor import PercentageTraceProcessor
...
class Simulator(SimulatorInterface):
    def __init__(self, network_file, service_functions_file, config_file, resource_functions_path="", test_mode=False,
        ...
        self.metrics = Metrics(self.network, self.sf_list, self.sfc_list)

        # self.percentage = {'pop0': {'sfc_1': 0.7, 'sfc_2':1}}
        # Assume result path is the path where network file is in.
        self.result_base_path = os.path.dirname(self.network_file)
        if 'percentage_path' in self.config:
            # Quick solution to copy trace file to same path for network file as provided by calling algo.
            percentage_path = os.path.join(os.getcwd(), self.config['percentage_path'])
            copyfile(percentage_path, os.path.join(self.result_base_path, os.path.basename(percentage_path)))
        ...
        if 'percentage_path' in self.config:
            percentage_path = os.path.join(os.getcwd(), self.config['percentage_path'])
            self.percentage_trace = reader.get_trace(percentage_path)
    ...
    def init(self, seed):
        ...
        if 'percentage_path' in self.config:
            logger.info(f"fuckin here")
            PercentageTraceProcessor(self.params, self.env, self.percentage_trace)
        else:
            self.percentage_trace_default()

        logger.info(f"self.params.percentage: {self.params.percentage}")
        # Start the simulator
        self.simulator.start()

        # TODO: Create runner here
        controller_cls = eval(self.params.controller_class)
        logger.info(f"self.params.controller_class: {self.params.controller_class}")
    ...
    def apply(self, actions):
        ...
        # set time stamp to calculate runtime of next apply call
        self.last_apply_time = time.time()
        logger.debug(f"self.params.metrics={json.dumps(self.params.metrics.metrics, indent = 2)}")
        return simulator_state
    ...
    def percentage_trace_default(self):
        """
        Get the default value for percentage
        """
        for idx,node in enumerate(self.params.ing_nodes):
            self.params.percentage[node[0]] = dict()
            for idx_key,key in enumerate(self.sfc_list):
                self.params.percentage[node[0]][key] = float((idx_key + 1)/len(self.sfc_list))
            
        logger.debug(f"self.params.percentage is {self.params.percentage}")
        pass
...
if __name__ == "__main__":
    ...
    network_file = 'res/networks/tue_network.graphml'
    service_file = 'res/service_functions/abc.yaml'
    config_file = 'res/config/simulator/test.yaml'
