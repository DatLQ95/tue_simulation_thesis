...
class Metrics:
    def __init__(self, network, sfs, sfcs):
        self.metrics = {}
    ...
    def reset_metrics(self):
        ...
        self.metrics['generated_flows_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['processed_flows'] = 0
        self.metrics['processed_flows_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['dropped_flows'] = 0
        self.metrics['dropped_flows_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['total_active_flows'] = 0
        self.metrics['total_active_flows_sfcs'] = {v: 0 for v in self.sfcs.keys()}
        ...
        self.metrics['total_processing_delay_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['num_processing_delays'] = 0
        self.metrics['num_processing_delays_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['avg_processing_delay'] = 0.0
        self.metrics['avg_processing_delay_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['total_path_delay'] = 0.0
        self.metrics['total_path_delay_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['num_path_delays'] = 0
        self.metrics['num_path_delays_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        # avg path delay per used path, not per entire service chain
        self.metrics['avg_path_delay'] = 0.0
        self.metrics['avg_path_delay_sfcs'] = {v: 0 for v in self.sfcs.keys()}
        
        self.metrics['total_end2end_delay'] = 0.0
        self.metrics['total_end2end_delay_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['avg_end2end_delay'] = 0.0
        self.metrics['avg_end2end_delay_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['avg_total_delay'] = 0.0
        self.metrics['avg_total_delay_sfcs'] = {v: 0 for v in self.sfcs.keys()}
    ...
    def reset_run_metrics(self):
        ...
        self.metrics['run_dropped_flows_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['run_end2end_delay'] = 0
        self.metrics['run_end2end_delay_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['run_avg_end2end_delay'] = 0.0
        self.metrics['run_avg_end2end_delay_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['run_max_end2end_delay'] = 0.0
        self.metrics['run_max_end2end_delay_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['run_total_path_delay'] = 0
        self.metrics['run_total_path_delay_sfcs'] = {v: 0 for v in self.sfcs.keys()}
        ...
        self.metrics['run_avg_path_delay_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['run_generated_flows'] = 0
        self.metrics['run_generated_flows_sfcs'] = {v: 0 for v in self.sfcs.keys()}
        
        self.metrics['run_in_network_flows'] = 0
        self.metrics['run_in_network_flows_sfcs'] = {v: 0 for v in self.sfcs.keys()}

        self.metrics['run_processed_flows'] = 0
        self.metrics['run_processed_flows_sfcs'] = {v: 0 for v in self.sfcs.keys()}
    ...
    def generated_flow(self, flow, current_node):
        ...
        self.metrics['generated_flows_sfcs'][flow.sfc] += 1

        self.metrics['run_generated_flows'] += 1
        self.metrics['run_generated_flows_sfcs'][flow.sfc] += 1
        
        self.metrics['total_active_flows'] += 1
        self.metrics['total_active_flows_sfcs'][flow.sfc] += 1
    ...
    def completed_flow_sfc(self, sfc):
        self.metrics['processed_flows_sfcs'][sfc] += 1
        self.metrics['run_processed_flows_sfcs'][sfc] += 1
        self.metrics['total_active_flows_sfcs'][sfc] -= 1
        assert self.metrics['total_active_flows_sfcs'][sfc] >= 0, "Cannot have negative active flows"

    def dropped_flow(self, flow, reason):
        flow.dropped = True
        self.metrics['dropped_flows'] += 1
        self.metrics['dropped_flows_sfcs'][flow.sfc] += 1
        ...
        self.metrics['run_dropped_flows_sfcs'][flow.sfc] += 1
        assert self.metrics['total_active_flows'] >= 0, "Cannot have negative active flows"
        assert self.metrics['total_active_flows_sfcs'][flow.sfc] >= 0, "Cannot have negative active flows"
    ...
    def add_processing_delay_sfc(self, delay, sfc):
        self.metrics['num_processing_delays_sfcs'][sfc] += 1
        self.metrics['total_processing_delay_sfcs'][sfc] += delay
        # self.metrics['num_processing_delays'] += 1
        # self.metrics['total_processing_delay'] += delay
    ...
    def add_path_delay_sfc(self, delay, sfc):
        self.metrics['num_path_delays_sfcs'][sfc] += 1
        self.metrics['total_path_delay_sfcs'][sfc] += delay

        # calc path delay per run; average over num generated flows in run
        self.metrics['run_total_path_delay_sfcs'][sfc] += delay
        if self.metrics['run_generated_flows_sfcs'][sfc] > 0:
            self.metrics['run_avg_path_delay_sfcs'][sfc] = self.metrics['run_total_path_delay_sfcs'][sfc] / self.metrics['run_generated_flows_sfcs'][sfc]
    ...
    def add_end2end_delay_sfc(self, delay, sfc):
        self.metrics['total_end2end_delay_sfcs'][sfc] += delay
        self.metrics['run_end2end_delay_sfcs'][sfc] += delay
        if delay > self.metrics['run_max_end2end_delay_sfcs'][sfc]:
            self.metrics['run_max_end2end_delay_sfcs'][sfc] = delay
    ...
    def calc_avg_processing_delay_sfc(self):
        for sfc in self.sfcs.keys():
            if self.metrics['num_processing_delays_sfcs'][sfc] > 0:
                self.metrics['avg_processing_delay_sfcs'][sfc] \
                    = self.metrics['total_processing_delay_sfcs'][sfc] / self.metrics['num_processing_delays_sfcs'][sfc]
            else:
                self.metrics['avg_processing_delay_sfcs'][sfc] = 0
    ...
    def calc_avg_path_delay_sfc(self):
        for sfc in self.sfcs.keys():
            if self.metrics['num_path_delays_sfcs'][sfc] > 0:
                self.metrics['avg_path_delay_sfcs'][sfc] = self.metrics['total_path_delay_sfcs'][sfc] / self.metrics['num_path_delays_sfcs'][sfc]
            else:
                self.metrics['avg_path_delay_sfcs'][sfc] = 0
    ...
    def calc_avg_end2end_delay_sfc(self):
        for sfc in self.sfcs.keys():
            # We divide by number of processed flows to get end2end delays for processed flows only
            if self.metrics['processed_flows_sfcs'][sfc] > 0:
                self.metrics['avg_end2end_delay_sfcs'][sfc] = self.metrics['total_end2end_delay_sfcs'][sfc] / self.metrics['processed_flows_sfcs'][sfc]
            else:
                self.metrics['avg_end2end_delay_sfcs'][sfc] = 0  # No avg end2end delay yet (no processed flows yet)

            if self.metrics['run_processed_flows_sfcs'][sfc] > 0:
                self.metrics[
                    'run_avg_end2end_delay_sfcs'
                ][sfc] = self.metrics['run_end2end_delay_sfcs'][sfc] / self.metrics['run_processed_flows_sfcs'][sfc]
            else:
                self.metrics['run_avg_end2end_delay_sfcs'][sfc] = 0  # No run avg end2end delay yet (no processed flows yet)
    ...
    def calc_avg_total_delay_sfc(self):
        for sfc in self.sfcs.keys():
            avg_processing_delay = self.metrics['avg_processing_delay_sfcs'][sfc]
            avg_path_delay = self.metrics['avg_path_delay_sfcs'][sfc]
            self.metrics['avg_total_delay_sfcs'][sfc] = np.mean([avg_path_delay, avg_processing_delay])
    ...
    def get_metrics(self):
        ...
        self.calc_avg_processing_delay_sfc()
        self.calc_avg_path_delay_sfc()
        self.calc_avg_total_delay_sfc()
        self.calc_avg_end2end_delay_sfc()
        return self.metrics
