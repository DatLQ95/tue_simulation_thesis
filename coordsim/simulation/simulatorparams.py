...
class SimulatorParams:
    def __init__(self, logger, network, ing_nodes, eg_nodes, sfc_list, sf_list, config, metrics, prediction=False,
        ...
        # Percentage of traffic for each SFCs
    ...
    def generate_flow_lists(self, now=0):
        ...
        for ing in ingress_ids:
            ...
            self.logger.info(f"generated_flows: {self.generated_flows}")
            self.logger.info(f"flow_arrival_list: {self.flow_arrival_list}")
            self.logger.info(f"flow_dr_list: {self.flow_dr_list}")
            self.logger.info(f"flow_size_list: {self.flow_size_list}")
